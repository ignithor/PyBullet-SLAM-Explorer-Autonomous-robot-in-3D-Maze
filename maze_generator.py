    ...
    def get_walls_to_build(self):
        ...
        for r in range(self.size):
            for c in range(self.size):
                center_x = c * cfg.CELL_SIZE + cfg.CELL_SIZE / 2
                center_y = r * cfg.CELL_SIZE + cfg.CELL_SIZE / 2
                ...
                if c < self.size - 1 and self.maze[r, c, 2] == 1:
                    pos_x = c * cfg.CELL_SIZE + cfg.CELL_SIZE + half_t
                    pos_y = center_y
                    half_extents = [half_t, cfg.CELL_SIZE / 2.0, cfg.WALL_HEIGHT / 2.0]
                    walls.append(( [pos_x, pos_y, cfg.WALL_HEIGHT / 2.0], half_extents, [0, 0, 0, 1] ))
                ...
                if r > 0 and self.maze[r, c, 0] == 1: 
                    pos_x = center_x
                    pos_y = r * cfg.CELL_SIZE - half_t
                    half_extents = [cfg.CELL_SIZE / 2.0, half_t, cfg.WALL_HEIGHT / 2.0]
                    walls.append(( [pos_x, pos_y, cfg.WALL_HEIGHT / 2.0], half_extents, [0, 0, 0, 1] ))
        ...
        for c in range(self.size):
            pos_x = c * cfg.CELL_SIZE + cfg.CELL_SIZE / 2
            pos_y = self.size * cfg.CELL_SIZE + half_t
            half_extents = [cfg.CELL_SIZE / 2.0 + half_t, half_t, cfg.WALL_HEIGHT / 2.0]
            walls.append(( [pos_x, pos_y, cfg.WALL_HEIGHT / 2.0], half_extents, [0, 0, 0, 1] ))
        ...
        for r in range(self.size):
            pos_x = self.size * cfg.CELL_SIZE + half_t
            pos_y = r * cfg.CELL_SIZE + cfg.CELL_SIZE / 2
            half_extents = [half_t, cfg.CELL_SIZE / 2.0 + half_t, cfg.WALL_HEIGHT / 2.0]
            walls.append(( [pos_x, pos_y, cfg.WALL_HEIGHT / 2.0], half_extents, [0, 0, 0, 1] ))
        ...
        # end_wall_x = (self.size - 1) * cfg.CELL_SIZE + cfg.CELL_SIZE / 2
        # end_wall_y = self.size * cfg.CELL_SIZE + half_t
